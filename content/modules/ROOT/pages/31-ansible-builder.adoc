...
+
image::31-ee-wizard-sidebar.png[EE wizard creator in VS Code,link=self,window=blank, opts="border"]
...
[source,bash,role=execute]
----
image-registry.openshift-image-registry.svc:5000/openshift/ee-minimal-rhel9:latest
----
+
NOTE: This is the Red Hat supported `ee-minimal` base image, pre-loaded into the cluster's internal registry. It is the same image used by Ansible Automation Platform in production.
...
+
TIP: The CLI equivalent of this wizard is: `ansible-creator init execution_env --ee-base-image image-registry.openshift-image-registry.svc:5000/openshift/ee-minimal-rhel9:latest --ee-collections mynamespace.mycollection --ee-python-deps cowsay --ee-name mycollection-ee /projects/ansible-dev-tools-workspace/mycollection-ee`
...
----
+
IMPORTANT: This command should display the documentation for your custom module, but we didn't add the corresponding metadata blocks! We will leave it to you as an optional task to research and fix this issue if you have time after finishing the lab. See the https://docs.ansible.com/projects/ansible/latest/dev_guide/developing_modules_documenting.html[Module format and documentation^] guide for details on how to add `DOCUMENTATION`, `EXAMPLES`, and `RETURN` blocks to your module.

---
== Next steps

You have successfully packaged your automation content into an Execution Environment using `ansible-builder` and VS Code. This image is now ready for upload to an Automation Hub or deployment via Automation Controller. Please click the **Next** button below to proceed to xref:32-ansible-sign.adoc[Lab 3.2 - Introducing supply chain security with ansible-sign].
