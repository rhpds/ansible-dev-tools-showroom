...
----
+
* **Plugin type:** Select `Filter` from the dropdown.
* **Plugin name:**
+
[source,text,role=execute]
----
...
+
image::21-myfilter-plugin-created.png[myfilter.py created in the plugins filter directory,link=self,window=blank, opts="border"]
+
...
.   **Create a default Molecule configuration.** In the VS Code Explorer sidebar, **expand** the `extensions/` folder, then **right-click** on the `molecule/` folder inside it and select **New File**. Name it:
+
[source]
----
