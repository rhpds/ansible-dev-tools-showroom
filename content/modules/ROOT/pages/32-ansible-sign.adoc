...
. *Copy the playbook file.* In the VS Code file explorer, expand `mynamespace.mycollection` > `playbooks`, **right-click** on `mycowsay.yml` and select **Copy**. Then **right-click** on the `mysignedplaybook` folder and select **Paste**, or use the terminal command below:
+
[source,bash,role=execute]
----
cp /projects/ansible-dev-tools-workspace/mynamespace.mycollection/playbooks/mycowsay.yml /projects/ansible-dev-tools-workspace/mysignedplaybook/
----
...
You have successfully signed and verified your playbook, ensuring that it can't be tampered with and continue to run unnoticed. Please click the **Next** button below to proceed to the xref:99-conclusion.adoc[Conclusion].
